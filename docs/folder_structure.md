...
|-- data/                   # 数据源：akshare/yfinance/RSS + 本地情绪CSV缓存
|-- model/                  # 时序建模：GARCH/ARIMA/LSTM/Transformer/高频RV
...
| `data/` | 数据获取与缓存（双分支设计） | `scrapers/market_data.py`, `sentiment_cache/local_sentiment.csv` |
| `model/` | 波动率/收益率时序模型 | `garch.py`, `arima_model.py`, `lstm_vol.py`, `transformer_model.py`, `highfreq.py`, `realized_vol.py` |
...
| `docs/` | 研究文档 | `research_report.md`, `limitations_future_work.md`, `demo_script.md` |
